...
def classify_input(state): # LLM 한 번 호출로 아래 2단계 작업 수행
    ...
    1. 당신은 일자리 챗봇입니다. 다음 작업을 수행하세요.
       1단계 : 사용자 입력이 아르바이트/알바/일자리와 관련된 내용인지 판단해서
    ...
    3. 중요 규칙
      ...
      6) 근무종료시각(end_time)
        - hh:mm이 표준
        - 예6) 점심시간 : "14:00"
      7) 시급(hourly_wage)은 알바 입장에서는 사실상 특정 시급 이상만 원하므로 아래와 같은 형식으로 저장
        - 숫자 또는 숫자 + "이상"
        - 숫자 다음의 화폐 단위는 무시 (나중에 '원(KW)'으로만 계산할 예정)
      8) 희망하는 알바/일자리의 업종/카테고리(category)는 아래에서 하나만 선택
        - {', '.join(CATEGORIES)}
